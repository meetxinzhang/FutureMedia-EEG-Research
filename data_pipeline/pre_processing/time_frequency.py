...
def signal2spectrum(signal, time=None, width=None):
    ...
    plt.imshow(cwtmatr, extent=[1, 2868, 12, 28], cmap='PRGn', aspect='auto',
               vmax=abs(cwtmatr).max(), vmin=-abs(cwtmatr).max())
    plt.show()
    return cwtmatr


def signal2spectrum_pywt_cwt(signal, scales=np.arange(1, 97)):
    ...
    # plt.imshow(image)
    plt.imshow(cwtmatr, extent=[1, 2868, 1, 97], cmap='PRGn', aspect='auto',
               vmax=abs(cwtmatr).max(), vmin=-abs(cwtmatr).max())
    plt.show()
    return cwtmatr
