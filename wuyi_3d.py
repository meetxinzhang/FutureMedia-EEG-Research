...
def main_func(gpu_rank, device_id, fold_rank,
              model, id_exp, train_dataset: AdaptedListDataset3d, valid_dataset: AdaptedListDataset3d):
    ...
    if model == 'VideoTsfm':
        ff = ViViT(h=20, w=20, t=128, patch_h=4, patch_w=4, patch_t=8, num_classes=40, channels=1,
                   dim=64, depth=2, heads=4, mlp_dim=32).to(the_device)
...
if __name__ == '__main__':
    ...
    # models = ['EEGNet', 'eegTsfm', 'lstm', 'mlp', '1dcnn', 'syncnet', 'resnet2d', 'resnet1d']
    models = ['ConvTsfm', 'VideoTsfm']
    exps = ['dct2d', 'adct', 'dff_b']
    # exps = ['ascode']

    for m in models:
        for exp in exps:
            _id_exp = id_exp + '--' + m + '-' + exp
            # if m == 'lstm' and exp in ['nm', 'dct2d', 'adct', 'ave', 't_dff']:
            #     continue
            ...
            dataset = AdaptedListDataset3d(filepaths, exp=exp, model=m)
